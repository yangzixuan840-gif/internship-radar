...
- 46 个国内中大型公司官网入口；为逐家公司实现低频、公开接口优先的 connector 预留数据模型；
...
当前可同步的来源是字节跳动、腾讯、美团、小红书、京东、滴滴、百度、哔哩哔哩、网易、携程、科大讯飞、地平线、小鹏汽车和蔚来。每个 connector 都只访问对应官网提供的未登录职位接口，大部分单次最多读取 100 条（滴滴为 6 页上限、百度为 20 条上限），并对每家公司设置 20 分钟的本地同步冷却时间。新增华为、小米、快手、拼多多、OPPO、vivo、新浪微博、爱奇艺、得物、顺丰科技、菜鸟、同程旅行，以及 DeepSeek、月之暗面、智谱 AI、MiniMax、阶跃星辰、百川智能、寒武纪、旷视科技等官网来源；它们会显示为“待适配”，只有完成接口验证后才会启用同步。优先顺序是：公开职位 API → 官网公开页面 → 人工导入。不会尝试绕过登录、验证码、反爬或网站访问限制。
