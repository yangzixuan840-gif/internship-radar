...
当前可同步的来源是字节跳动、腾讯、美团、小红书、京东、滴滴、百度、哔哩哔哩、网易、携程、科大讯飞、地平线、小鹏汽车和蔚来。每个 connector 都只访问对应官网提供的未登录职位接口，大部分单次最多读取 100 条（滴滴为 6 页上限、百度为 20 条上限），并对每家公司设置 20 分钟的本地同步冷却时间。商汤、理想、极氪、海康、微众和平安等公司的官网入口也已纳入来源清单；只有完成接口验证后才会启用同步。优先顺序是：公开职位 API → 官网公开页面 → 人工导入。不会尝试绕过登录、验证码、反爬或网站访问限制。
