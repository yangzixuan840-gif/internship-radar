...
- 已接入字节跳动、腾讯、美团的公开实习职位 API；点击“同步三家官网”即可手动增量同步；
...
当前可同步的来源是字节跳动、腾讯和美团。每个 connector 都只访问对应官网提供的未登录职位接口，单次最多读取 100 条，并对每家公司设置 20 分钟的本地同步冷却时间。优先顺序是：公开职位 API → 官网公开页面 → 人工导入。不会尝试绕过登录、验证码、反爬或网站访问限制。
