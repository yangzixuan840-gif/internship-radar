...
- 已接入字节跳动、腾讯、美团、小红书、京东、滴滴、百度、哔哩哔哩、网易的公开职位 API；点击“同步已接入官网”即可手动增量同步；
...
当前可同步的来源是字节跳动、腾讯、美团、小红书、京东、滴滴、百度、哔哩哔哩和网易。每个 connector 都只访问对应官网提供的未登录职位接口，大部分单次最多读取 100 条（滴滴为 6 页上限、百度为 20 条上限），并对每家公司设置 20 分钟的本地同步冷却时间。优先顺序是：公开职位 API → 官网公开页面 → 人工导入。不会尝试绕过登录、验证码、反爬或网站访问限制。
